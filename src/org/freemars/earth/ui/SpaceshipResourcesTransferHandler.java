...
public class SpaceshipResourcesTransferHandler extends PatchedTransferHandler {
    ...
    public Icon getVisualRepresentation(Transferable transferable) {
        try {
            ...
            return new ImageIcon(FreeMarsImageManager.getInstance().getImage(resource));
        } catch (Exception e) {
        }
        return new ImageIcon(FreeMarsImageManager.getInstance().getImage("RESOURCE_1"));
    }
    ...
    protected Transferable createTransferable(JComponent c) {
        ...
        int unitResourceQuantity = freeMarsController.getFreeMarsModel().getEarthFlightModel().getEarthDialogSelectedUnit().getResourceQuantity(resource);
        ...
        try {
            ...
            if (player.hasDeclaredIndependence()) {
                ...
                if (earthTransferData.getSource() == EarthTransferable.FROM_EARTH) {
                    ...
                    int unitResourceRemainingCapacity = freeMarsController.getFreeMarsModel().getEarthFlightModel().getEarthDialogSelectedUnit().getRemainingCapacity(resource);
                    quantity = quantity > unitResourceRemainingCapacity ? unitResourceRemainingCapacity : quantity;
                    int totalPrice = freeMarsController.getFreeMarsModel().getEarthFlightModel().getEarthSellsAtPrice(resource) * quantity;
                    String formattedTotalPrice = new DecimalFormat().format(totalPrice);
                    String formattedQuantity = new DecimalFormat().format(quantity);
                    CommandResult result = freeMarsController.execute(new BuyResourceFromEarthCommand(freeMarsController.getFreeMarsModel().getEarthFlightModel(), (Unit) resourceStorer, resource, quantity));
                    if (result.getCode() == CommandResult.RESULT_OK) {
                        ...
                        earthDialog.update(EarthFlightModel.BUY_SELL_UPDATE);
                    }
                }
            }
