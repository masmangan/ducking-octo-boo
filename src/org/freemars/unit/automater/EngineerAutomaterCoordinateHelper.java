...
public class EngineerAutomaterCoordinateHelper {
    ...
    protected Path getPath(Coordinate coordinateA, Coordinate coordinateB) {
        ...
        if (path == null) {
            path = freeMarsController.getFreeMarsModel().getRealm().getPathFinder().findPath(unit, coordinateA, coordinateB, false);
            if (path != null) {
    ...
    private boolean engineerNearCoordinate(Player player) {
        UnitType engineerUnitType = freeMarsController.getFreeMarsModel().getRealm().getUnitTypeManager().getUnitType(1);
