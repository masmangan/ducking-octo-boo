...
public class RhombusGridPanel extends JPanel {
    ...
    public void paintGridlines(Graphics g) {
        ...
        for (int ordinate = 0; ordinate < verticalGrids; ordinate++) {
            for (int abscissa = 0; abscissa < horizontalGrids; abscissa++) {
                Coordinate coordinate = new Coordinate(abscissa, ordinate);
                Rectangle rectangle = new Rectangle(getRhombusPoint(coordinate), new Dimension(gridHeight, gridHeight));
                paintRhombus(g2d, getRhombusPoint(coordinate));
                g2d.drawString(coordinate.toString(), (int) rectangle.getX() + 10, ((int) rectangle.getY()) + (gridHeight / 2));
            }
        }
    }
