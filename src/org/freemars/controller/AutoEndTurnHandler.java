 ...
 * @author Deniz ARIKAN
 */
public class AutoEndTurnHandler implements ControllerUpdateHandler {
    ...
    private boolean isThereAManageableSpaceshipInAColony(FreeMarsPlayer freeMarsPlayer) {
        ...
        while (unitsIterator.hasNext()) {
            Unit playerUnit = unitsIterator.next();
            if (playerUnit.getStatus() == Unit.UNIT_ACTIVE && playerUnit.getType().getProperty("EarthFlight") != null && !playerUnit.isSkippedForCurrentTurn()) {
                return true;
            }
        }
        return false;
