 ...
 * @author Deniz ARIKAN
 */
public class PlayerRemovedHandler implements ControllerUpdateHandler {
    ...
    public void handleUpdate(FreeMarsController controller, CommandResult commandResult) {
        ...
        if (!removedPlayer.equals(model.getHumanPlayer())) {
            if (isExpeditionaryForceDefeated(model, removedPlayer)) {
                ...
                message.setText(new StringBuffer("We have defeated the " + removedPlayer));
                ...
                message.setSubject("Player defeated");
                message.setText(new StringBuffer(removedPlayer.getNation().getCountryName() + " has withdrawn from colonization of Mars"));
                message.setTurnSent(model.getNumberOfTurns());
